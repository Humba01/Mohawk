...
public class LithiumServerTickScheduler<T> extends ServerTickList<T> {
    ...
    private List<NextTickListEntry<T>> collectTicks(MutableBoundingBox bb, boolean remove, Predicate<TickEntry<?>> predicate) {
        List<NextTickListEntry<T>> result = new ArrayList<>();

        int minChunkX = bb.minX >> 4;
        int maxChunkX = bb.maxX >> 4;

        int minChunkZ = bb.minZ >> 4;
        int maxChunkZ = bb.maxZ >> 4;
        ...
        for (int chunkX = minChunkX; chunkX <= maxChunkX; chunkX++) {
            for (int chunkZ = minChunkZ; chunkZ <= maxChunkZ; chunkZ++) {
                ...
                for (TickEntry<T> tick : set) {
                    BlockPos blockPos = tick.position;

                    // [VanillaCopy] ServerTickList#getEntries
        ...
        if (remove) {
            for (NextTickListEntry<T> tick : result) {
