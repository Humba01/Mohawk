    ...
    public static boolean isMethodFromSuperclassOverwritten(Class<?> clazz, Class<?> superclass, String methodName, Class<?>... methodArgs) {
        while (clazz != null && clazz != superclass && superclass.isAssignableFrom(clazz)) {
            try {
                ...
                CrashReport crashReport = CrashReport.makeCrashReport(e, "Lithium EntityClassGroup analysis");
                CrashReportCategory crashReportCategory = crashReport.makeCategory(e.getClass().toString() + " when getting declared methods.");
                crashReportCategory.addDetail("Analyzed class", crashedClass);
                crashReportCategory.addDetail("Analyzed method name", methodName);
                crashReportCategory.addDetail("Analyzed method args", methodArgs);
